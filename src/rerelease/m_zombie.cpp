...
MMOVE_T(zombie_move_walk) = {15, 33, zombie_frames_walk, NULL};
...
TOUCH(zombie_gib_touch) (edict_t* self, edict_t* other, const trace_t& tr, bool other_touching_self) -> void
{
	vec3_t point;
	vec3_t normal;
	int	   damage;
...
void FireZombieGib(edict_t *self)
{
	...
	//VectorNormalize(dir);
	dir.normalize();
	fire_zombie_gib(self, start, vec, 10, 1200);
}
