...
MMOVE_T(fiend_prototype_move_stand) = {0, 12, fiend_prototype_frames_stand, NULL};

MONSTERINFO_STAND(fiend_prototype_stand) (edict_t *self) -> void
{
...
MMOVE_T(fiend_prototype_move_run) = {21, 26, fiend_prototype_frames_run, NULL};

MONSTERINFO_RUN(fiend_prototype_run) (edict_t *self) -> void
{
...
MMOVE_T(fiend_prototype_move_walk) = { 13, 20, fiend_prototype_frames_walk, NULL };

MONSTERINFO_WALK(fiend_prototype_walk) (edict_t* self) -> void
{
	M_SetAnimation(self, &fiend_prototype_move_walk);
}

bool CheckfiendPrototypeJump(edict_t *self)
{
	vec3_t	dir;
	float	distance;
...
void fiendPrototypeJump(edict_t *self)
{
	vec3_t forward;
	...
	self->touch = fiendPrototypeJumpTouch;
}

void fiend_prototype_roar(edict_t *self)
{
...
MMOVE_T(fiend_prototype_move_jump) = {27, 38, fiend_prototype_frames_jump, fiend_prototype_run};

MONSTERINFO_ATTACK(fiend_prototype_attack) (edict_t *self) -> void
{
	M_SetAnimation(self, &fiend_prototype_move_jump);	
}

void fiendPrototypeMelee(edict_t *self)
{
	vec3_t dir;
...
// Melee
mframe_t fiend_prototype_frames_melee [] =
{
	{ai_charge, 4,	NULL},
	{ai_charge, 0,	NULL},
	{ai_charge, 0,	NULL},
	{ai_charge, 1,	NULL},
	{ai_charge, 14,	fiendPrototypeMelee},
	{ai_charge, 1,	NULL},
	{ai_charge, 6,	NULL},
	{ai_charge, 8,	NULL},
	{ai_charge, 4,	NULL},
	{ai_charge, 2,	NULL},
	{ai_charge, 12,	fiendPrototypeMelee},
	{ai_charge, 5,	NULL},
	{ai_charge, 8,	NULL},
	{ai_charge, 4,	NULL},
	{ai_charge, 4,	NULL}
};
MMOVE_T(fiend_prototype_move_melee) = {54, 68, fiend_prototype_frames_melee, fiend_prototype_run};

MONSTERINFO_MELEE(fiend_prototype_melee) (edict_t *self) -> void
{
...
MMOVE_T(fiend_prototype_move_pain) = {39, 44, fiend_prototype_frames_pain, fiend_prototype_run};

PAIN (fiend_prototype_pain)  (edict_t* self, edict_t* other, float kick, int damage, const mod_t& mod) -> void
{
	if (skill->value == 3)
		return;
	if (self->touch = fiendPrototypeJumpTouch)
		return;
...
MONSTERINFO_SETSKIN(fiend_prototype_setskin) (edict_t* self) -> void
{
...
void fiend_prototype_dead(edict_t *self)
{
	self->mins = { -32, -32, -24 };
	self->maxs = { 32, 32, -8 };
...
MMOVE_T(fiend_prototype_move_die) = {45, 53, fiend_prototype_frames_die, fiend_prototype_dead};

DIE(fiend_prototype_die) (edict_t* self, edict_t* inflictor, edict_t* attacker, int damage, const vec3_t& point, const mod_t& mod) -> void
{
...
void SP_monster_fiend_prototype(edict_t *self)
{
	...
	self->monsterinfo.attack = fiend_prototype_attack;
	self->monsterinfo.melee = fiend_prototype_melee;
