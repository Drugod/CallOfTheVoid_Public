...
//DRUGOD
THINK(misc_model_anim_think) (edict_t* self) -> void {
	self->s.frame++;

	if (self->s.frame >= self->s.max_frames) {
		self->s.frame = 0;
	}
...
void SP_misc_model_anim(edict_t* ent) {
	gi.setmodel(ent, ent->model);
	...
	ent->think = misc_model_anim_think;
	ent->nextthink = level.time + 0.1_sec;
	gi.linkentity(ent);
}

THINK(misc_fiend_craft_think) (edict_t* self) -> void
{
	self->s.frame++;
	if (self->s.frame >= 60)
		self->s.frame = 0;
...
USE(misc_fiend_craft_use) (edict_t* self, edict_t* other, edict_t* activator) -> void
{
...
void SP_misc_fiend_craft(edict_t* ent)
{
	...
	ent->maxs = { 64, 64, 128 };
	ent->s.frame = 0;
	ent->s.modelindex = gi.modelindex("models/props/fiend/tris.md2");
	if (ent->targetname) {
...
THINK(light_flame_small_think) (edict_t* ent) -> void
{
	ent->s.frame++;
	if (ent->s.frame >= 11)
		ent->s.frame = 0;
...
void SP_light_flame_small(edict_t* ent)
{
	...
	ent->solid = SOLID_NOT;
	ent->svflags = SVF_DEADMONSTER;
	ent->s.modelindex = gi.modelindex("models/props/flame/tris.md2");
	ent->think = light_flame_small_think;
	ent->nextthink = level.time + 0.1_sec;
	gi.linkentity(ent);
}

/*QUAKED light_flame_torch (1 0.5 0) (-4 -4 -8) (4 4 8)
*/

THINK(light_flame_torch_think) (edict_t* ent) -> void
{
	ent->s.frame++;
	if (ent->s.frame >= 6)
		ent->s.frame = 0;
...
void SP_light_torch_small(edict_t* ent)
{
	...
	ent->solid = SOLID_NOT;
	ent->svflags = SVF_DEADMONSTER;
	ent->s.modelindex = gi.modelindex("models/props/torch/tris.md2");
	ent->think = light_flame_torch_think;
	ent->nextthink = level.time + 0.1_sec;
	gi.linkentity(ent);
}