...
/*QUAKED trigger_key2 (.5 .5 .5) (-8 -8 -8) (8 8 8) SILENT
A relay trigger that only fires its targets if player has the proper key.
Use "item" to specify the required key, for example "key_data_cd".
Spawnflag 1 (SILENT) disables the missing key message and sound.
Spawnflag 2 (SILENT) disables activation sound.
*/

constexpr spawnflags_t SPAWNFLAG_KEY_NOITEMREMOVE = 4_spawnflag;
constexpr spawnflags_t SPAWNFLAG_KEY_NOACTIVATIONSOUND = 2_spawnflag;
constexpr spawnflags_t SPAWNFLAG_KEY_NOMESSAGE = 1_spawnflag;

USE(trigger_key2_use) (edict_t* self, edict_t* other, edict_t* activator) -> void
{
	item_id_t index;
	...
	if (!activator->client->pers.inventory[index])
	{
		...
		// Si NO está activado el spawnflag 1 (SILENT), mostrar mensaje y sonido
